...
export interface TimeInStatus { [status: string]: number }
...
export interface AnnualReportTask {
  id: string; name: string; entity_name: string
  due_date: string; filed_date: string | null
  status: AnnualReportStatus; state: StateType
  package: PackageType; assignee: string
}
export type AgenteStatus = "pendiente" | "esperando_invoice" | "completado"
export interface AgenteRegistradoTask {
  id: string; name: string; entity_name: string
  state: StateType; package: PackageType
  renewal_date: string; status: AgenteStatus; assignee: string
}
export interface CXTicket {
  ...
  state: StateType; package: PackageType; assignee: string
}
...
function mapToTask(raw: any): Task | null {
  ...
  const fechaCreacion      = getCustomFieldValue(cf, "fecha_creacion")      ?? msToDate(parseInt(raw.date_created))!
  const envioTramite       = getCustomFieldValue(cf, "envio_tramite")       ?? getCustomFieldValue(cf, "envío tramite")
  const fechaSolicitudEin  = getCustomFieldValue(cf, "fecha_solicitud_ein") ?? getCustomFieldValue(cf, "fecha solicitud ein")
  const fechaRecepcionEin  = getCustomFieldValue(cf, "fecha_recepcion_ein") ?? getCustomFieldValue(cf, "fecha recepcion ein")

  const einStatusRaw = getCustomFieldValue(cf, "ein_status") ?? getCustomFieldValue(cf, "ein status") ?? "n/a"
  const einStatus: EINStatus =
    ["pendiente","solicitado","recibido","n/a"].includes(einStatusRaw.toLowerCase())
...
function mapToBankTask(raw: any): BankTask {
  ...
  const BANK_STATUSES: BankStatus[] = [
    "PENDIENTE","ESPERANDO EIN","ESPERANDO INPUT CLIENTE",
    "INFO ADICIONAL BANK","VERIF IDENTIDAD","INICIADA",
  ]

  // Extraccion de campos personalizados exactos de "Aplicaciones 2.0"
  const fechaCreacion    = getCustomFieldValue(cf, "fecha de creaci��n") ?? getCustomFieldValue(cf, "fecha de creacion") ?? msToDate(parseInt(raw.date_created))!
  const solicitudInfo    = getCustomFieldValue(cf, "solicitud info")
  const fechaCorreccion  = getCustomFieldValue(cf, "fecha corrección") ?? getCustomFieldValue(cf, "fecha correccion")
  const fechaAplicacion  = getCustomFieldValue(cf, "fecha aplicación") ?? getCustomFieldValue(cf, "fecha aplicacion")
  const pedidoVerifId    = getCustomFieldValue(cf, "pedido verif. id") ?? getCustomFieldValue(cf, "pedido verif id")
  const completaVerifId  = getCustomFieldValue(cf, "completa verif. id") ?? getCustomFieldValue(cf, "completa verif id")
  const fechaAprobRech   = getCustomFieldValue(cf, "fecha aprob/rech") ?? getCustomFieldValue(cf, "fecha aprob rech")
  ...
  const blockingAlert: "client_blocked" | "bank_delay" | null =
    (status === "ESPERANDO INPUT CLIENTE" && currentDays > 3) ? "client_blocked"
    : ((status === "VERIF IDENTIDAD" || status === "INFO ADICIONAL BANK") && currentDays > 7) ? "bank_delay"
    : null
...
export async function getFilteredAnnualReports(
  _state?: StateType | "all", _pkg?: PackageType | "all"
): Promise<AnnualReportTask[]> { return [] }

export async function getFilteredAgentesRegistrados(
  _state?: StateType | "all", _pkg?: PackageType | "all"
): Promise<AgenteRegistradoTask[]> { return [] }

export async function getFilteredCXTickets(
  _state?: StateType | "all", _pkg?: PackageType | "all"
): Promise<CXTicket[]> { return [] }

// ─── KPI CALCULATORS (idénticos a mock-data.ts) ────────────

export function getFunnelData(tasks: Task[]) {
  const counts: Record<LLCStatus, number> = {
    PENDIENTE: 0, "NO INICIAR": 0, "ESPERANDO INPUT CLIENTE": 0,
    "ESPERANDO APROB": 0, "APROBADA EN ESTADO": 0, "1º ENTREGA DOCS": 0,
    FAXEADO: 0, "ESPERANDO EIN": 0, "EIN LISTO": 0,
    CANCELADO: 0, "ENTREGA COMPLETADA": 0,
  }
...
export function getAverageTimeByStatus(tasks: Task[]) {
  const times: Record<LLCStatus, number[]> = {
    PENDIENTE: [], "NO INICIAR": [], "ESPERANDO INPUT CLIENTE": [],
    "ESPERANDO APROB": [], "APROBADA EN ESTADO": [], "1º ENTREGA DOCS": [],
    FAXEADO: [], "ESPERANDO EIN": [], "EIN LISTO": [],
    CANCELADO: [], "ENTREGA COMPLETADA": [],
  }
  tasks.forEach((t) =>
...
export function calculateLeadTime(tasks: Task[]) {
  ...
  return Math.round(
      ...
      return s + Math.ceil((new Date(t.closed_at!).getTime() - new Date(t.created_at).getTime()) / 86400000)
    }, 0) / done.length
  )
}
...
export function calculateCycleTimeKPIs(tasks: Task[]) {
  ...
  const avgEINWait      = einWait.length
    ? Math.round(einWait.reduce((s, t) => s + (t.time_in_status["ESPERANDO EIN"] ?? 0), 0) / einWait.length)
    : 0
  const delayedTasks = tasks.filter((t) =>
    (["ESPERANDO INPUT CLIENTE","ESPERANDO APROB","FAXEADO","ESPERANDO EIN"] as LLCStatus[]).some(
      (st) => (t.time_in_status[st] ?? 0) > 5
    )
  ).length
  return { totalTasks, completedTasks, cancelledTasks, inProgressTasks, avgLeadTime, avgEINWait, delayedTasks }
}
...
export function calculateBankKPIs(tasks: BankTask[]) {
  ...
  const avgLeadTime = closedTasks.length > 0
    ? Math.round(closedTasks.reduce((sum, t) => {
...
export function getBankStatusCounts(tasks: BankTask[]) {
  ...
  const counts: Record<BankStatus, number> = {
    PENDIENTE: 0, "ESPERANDO EIN": 0, "ESPERANDO INPUT CLIENTE": 0,
    "INFO ADICIONAL BANK": 0, "VERIF IDENTIDAD": 0, INICIADA: 0,
  }
  ...
  return BANK_DISPLAY_STATUSES.map((s) => ({ status: s, count: counts[s], color: BANK_STATUS_COLORS[s] }))
}
